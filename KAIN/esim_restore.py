...
def fill_placeholder(data, model,config):
  ...
  feature_x=np.reshape(feature_x,(config.batch_size,config.xmaxlen,config.ymaxlen,5)) 
...
def run_epoch(session, data,model,config, eval_op=None, verbose=False):
  ...
  for step in range(epoch_size):
    ...
    #v1_all.append(np.split(vals["v"],2)[0])
    #v2_all.append(np.split(vals["v"],2)[1])
    cos_dis.extend(cosine_distance(v1=np.split(vals["v"][0],2)[0], v2 =np.split(vals["v"][0],2)[1] ))
    #print ("v",vals["v"][0])
    #print ("v1",np.split(vals["v"][0],2)[0])
    #print ("v2",np.split(vals["v"][0],2)[1])
  ...
  label_input(true_label=true_label_total,
              pred_label=pred_label_total,
              pred_prob =pred_prod_total,
              cos_dis=cos_dis,
              input_file="test_5labels.txt",
              output_file="label_test.txt")   

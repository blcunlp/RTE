...
'''
def read_lemma_data(xmaxlen,ymaxlen,path):
  lemma_X,lemma_Y=[],[]
  with open(path) as f:
    for line in f:
      l = line.strip().split('\t')
      lemma_p=l[0].split(' ')
      lemma_p_f=filter_length(lemma_p,xmaxlen)
      lemma_h=l[1].split(' ')
      lemma_h_f=filter_length(lemma_h,ymaxlen)
      lemma_X+=[lemma_p_f]
      lemma_Y+=[lemma_h_f]
  return lemma_X,lemma_Y
...
class DataSet(object):
  def __init__(self,x,y,labels,x_len,y_len,X_mask,Y_mask,triple,mask_triple_inside,mask_triple_outside):
    ...
    self._y_mask=Y_mask
    self.triple=triple
    self.mask_triple_inside=mask_triple_inside
    self.mask_triple_outside=mask_triple_outside
  ...
  def next_batch(self, batch_size):
    ...
    batch_mask_triple_inside=self.mask_triple_inside[start:end]
    batch_mask_triple_outside=self.mask_triple_outside[start:end]
    return batch_x,batch_y, batch_labels,batch_x_mask,batch_y_mask,batch_x_len,batch_y_len,batch_triple,batch_mask_triple_inside,batch_mask_triple_outside
  @property
  def get_x(self):
    return self._x
...
def file2seqid(config):
  ...
  lemma_X_train,lemma_Y_train=read_lemma_data(xmaxlen,ymaxlen,config.lemma_train_file)
  lemma_X_dev,lemma_Y_dev=read_lemma_data(xmaxlen,ymaxlen,config.lemma_dev_file)
  lemma_X_test,lemma_Y_test=read_lemma_data(xmaxlen,ymaxlen,config.lemma_test_file)

  lemma_vocab=read_vocab("../data/lemma/lemma_vocab.txt")

  triple_dic=read_triple('../data/snli_1.0/triple_train.txt')
  rel2id_dic=relation2id('./relation2id.txt')

  train_triple_id,mask_train_triple_inside,mask_train_triple_outside=set_relation_id(config.xmaxlen,config.ymaxlen,lemma_X_train,lemma_Y_train,triple_dic,lemma_vocab,rel2id_dic)
  dev_triple_id,mask_dev_triple_inside,mask_dev_triple_outside=set_relation_id(config.xmaxlen,config.ymaxlen,lemma_X_dev,lemma_Y_dev,triple_dic,lemma_vocab,rel2id_dic)
  test_triple_id,mask_test_triple_inside,mask_test_triple_outside=set_relation_id(config.xmaxlen,config.ymaxlen,lemma_X_test,lemma_Y_test,triple_dic,lemma_vocab,rel2id_dic)
  '''
  ...
  Train = DataSet(X_train,Y_train,Z_train,X_train_lengths,Y_train_lengths,X_train_mask,Y_train_mask,train_triple_id,mask_train_triple_inside,mask_train_triple_outside)
  #batch_x,batch_y, batch_labels,batch_x_mask,batch_y_mask,batch_x_len,batch_y_len,batch_triple,batch_mask_triple=Train.next_batch(TestConfig.batch_size) 
  Dev = DataSet(X_dev,Y_dev,Z_dev,X_dev_lengths,Y_dev_lengths,X_dev_mask,Y_dev_mask,dev_triple_id,mask_dev_triple_inside,mask_dev_triple_outside)
  Test = DataSet(X_test,Y_test,Z_test,X_test_lengths,Y_test_lengths,X_test_mask,Y_test_mask,test_triple_id,mask_test_triple_inside,mask_test_triple_outside)

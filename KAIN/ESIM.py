...
from my.ops_cudnn_bilstm import *
#import my.esim_helper as esim_helper
from myutils import *
...
class SNLIModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    with tf.variable_scope("add_embedding") as scope:
      x_glove_emb,y_glove_emb = self.add_pretrained_embedding()

      input_xemb = x_glove_emb
      input_yemb = y_glove_emb

      if self.config.pos_tagging ==True:
        x_pos_emb, y_pos_emb = self.add_pos_embedding()
        input_xemb = tf.concat([x_glove_emb,x_pos_emb],axis=-1)
        input_yemb = tf.concat([y_glove_emb,x_pos_emb],axis=-1)
  ...
  def add_placeholder(self):
    ...
    self.x_mask = tf.placeholder(tf.int32, [self.config.batch_size, self.config.xmaxlen],name="x_mask")
    self.y_mask = tf.placeholder(tf.int32, [self.config.batch_size, self.config.ymaxlen],name="y_mask")
    self.x_mask = tf.cast(self.x_mask,tf.float32)
    self.y_mask = tf.cast(self.y_mask,tf.float32)

    self.x_len = tf.placeholder(tf.int32, [self.config.batch_size,],name="x_len")
    self.y_len = tf.placeholder(tf.int32, [self.config.batch_size,],name="y_len")
    self.x_len = tf.cast(self.x_len,tf.float32)
    self.y_len = tf.cast(self.y_len,tf.float32)

    self.x_pos = tf.placeholder(tf.int32, [self.config.batch_size, self.config.xmaxlen],name="x_pos")
    self.y_pos = tf.placeholder(tf.int32, [self.config.batch_size, self.config.ymaxlen],name="y_pos")

...
class SNLIModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    with tf.variable_scope("encode_x"):
      ...
      self.x_output, self.x_state = tf.nn.dynamic_rnn(cell=self.fwd_lstm_p, inputs=input_xemb,sequence_length=input_.x_len,dtype=tf.float32)
    ...
    with tf.variable_scope("encode_y"):
      ...
      self.y_output, self.y_state = tf.nn.dynamic_rnn(cell=self.fwd_lstm_h, inputs=input_yemb,sequence_length=input_.y_len,initial_state=self.x_state, dtype=tf.float32)
  ...
  def step_attention(self,ht,prev_r):

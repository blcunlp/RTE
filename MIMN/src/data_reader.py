...
def get_vocab(data):
    ...
    vocab_exist=os.path.isfile(config.vocab_dir)
    embedding_exist=os.path.isfile(config.glove_dir)
    ''' 
    if (not embedding_exist)or (config.rebuiltGloveEmb== True):
      print ("write vocab.txt")
      f =open("config.vocab_dir","w+")
      for x,y in enumerate(lst):
        x_y = str(y) +"\t"+ str(x)+"\n"
        f.write(x_y)
        #f.write("\n")
      f.close()
      print("create vocab successfully!\n")

      os.system("../../data/glove/sick_glove.sh")
      print("create sick_glove.npy successfully")
    '''
...
if __name__=="__main__":
    ...
    train=[l.strip().split('\t') for l in open(config.train_file)][:20000]
    dev=[l.strip().split('\t') for l in open(config.dev_file)]
    test=[l.strip().split('\t') for l in open(config.test_file)]
    labels={'neutral':0,'entailment':1,'contradiction':2}

    vocab=get_vocab(train)
    print("get_vocab?done")
    #X_train,Y_train,Z_train=load_data(train,vocab)
    X_dev,Y_dev,Z_dev=load_data(dev,vocab)

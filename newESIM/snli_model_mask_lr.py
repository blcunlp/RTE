...
class SNLIModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    with tf.device("/cpu:0"):
      embedding_matrix=np.load("../../data/glove/snli_embedding.npy")
    ...
    #with tf.variable_scope("encode_x",reuse=tf.AUTO_REUSE) as scope:
    with tf.variable_scope("encode_x") as scope:
      ...
      encode_cudnn_cell = cudnn_lstm(num_layers=1,hidden_size=self.config.hidden_units)
      self.x_output,_,_ = call_cudnn_lstm(inputs=input_xemb,num_layers=1,hidden_size=self.config.hidden_units,cudnn_cell=encode_cudnn_cell)    

      #scope.reuse_variables()
      #variable_scope.get_variable_scope().reuse_variables()
      #with tf.variable_scope("",reuse=True):
      #self.y_output,_,_ = cudnn_lstm(inputs=input_yemb,num_layers=1,hidden_size=self.config.hidden_units,is_training=self.is_training,is_reuse=True)    
      self.y_output,_,_ = call_cudnn_lstm(inputs=input_yemb,num_layers=1,hidden_size=self.config.hidden_units,cudnn_cell=encode_cudnn_cell)    
    ...
    with tf.variable_scope("composition"):
      ...
      #with tf.variable_scope("encode_mxy",reuse=tf.AUTO_REUSE) as scope:
      with tf.variable_scope("encode_mxy") as scope:
        ...
        encodexy_cudnn_cell = cudnn_lstm(num_layers=1,hidden_size=self.config.hidden_units)
        mxy_output,_,_ = call_cudnn_lstm(inputs=m_xy,num_layers=1,hidden_size=self.config.hidden_units,cudnn_cell=encodexy_cudnn_cell)    
        #scope.reuse_variables()
        myx_output,_,_ = call_cudnn_lstm(inputs=m_yx,num_layers=1,hidden_size=self.config.hidden_units,cudnn_cell=encodexy_cudnn_cell)    
        print("scope.global_variables:",scope.global_variables())

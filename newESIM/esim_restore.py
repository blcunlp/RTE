...
def run_epoch(session, data,model,config, eval_op=None, verbose=False):
  ...
  return acc_average,loss_average,global_step,learning_rate,true_label_total, pred_label_total
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as session:
      test_acc,test_loss,_,_,true,pred = run_epoch(session, data=Test,model=mtest,config=eval_config)
      out = open("esim_result.txt", "a")
      for m,n in zip(true,pred):
        #print(m,n)
        out.write(str(m)+"\t"+str(n)+"\n")
      print(true,pred)
